...
exportdir <- "/Users/mcallister/Desktop/03_Projects/03_Mitogenomes/gap_analysis/10_testing/"
inputfile <- "/Users/mcallister/Desktop/03_Projects/03_Mitogenomes/gap_analysis/10_testing/list_input.csv"
headerName <- "scientific_name" #This is the name of the header column in your tab-delimited text file where the query organism names are stored.
...
#For https://github.com/mooreryan/taxi use (subtree ids)
subtree_path <- "/usr/local/bin/taxi"
nodes_dmp_path <- "/Users/mcallister/software/blastdb/20241127_taxdump/nodes.dmp"

#For taxonomizr (this is the name of the output file to be created [if never run] or used)
taxa_outfile_ncbi <- "/Users/mcallister/software/blastdb/taxonomizr/20241206_ncbiTax/accessionTaxa.sql"

#BOLD Database Download
BOLD_local <- "/Users/mcallister/Desktop/03_Projects/03_Mitogenomes/gap_analysis/BOLD_dl/BOLD_Public.15-Aug-2025/BOLD_Public.15-Aug-2025.tsv"
...
input_list_clean <- read.delim("list_cleaned.txt", header=TRUE, stringsAsFactors=FALSE, na = c("","NA"))

scientific_name_table <- input_list_clean %>%
...
positive_control <- tibble(
  Scientific.Name = c("MICROPLANA SCHARFFI", "PEROMYSCUS MANICULATUS", "RHYNCHODEMUS SYLVATICUS", "BIPALIUM KEWENSE"),
  AphiaID = c("416391", "1597420", "483252", "416385"),
  NCBI_taxID = c("NCBI:txid391273", "NCBI:txid10042", "NCBI:txid1520503", "NCBI:txid66750"),
  Source = c("NA", "NA", "NA", "NA")  # Add more fields if needed
)
...
# DO NOT MODIFY SCIENTIFIC NAMES
...
#Check for positive control results in each chunk: MITO
```{r}
...
#Check for positive control results in each chunk: SRA
```{r}
chunk_files <- list.files("NCBI_search/taxIDs", pattern = "^NCBI_output_sra_0[356]_.*chunk.*\\.txt$", full.names = TRUE)

validate_chunk_sra <- function(chunk_file, positive_control) {
  chunk_df <- read_csv(chunk_file, col_types = cols(.default = col_character()), na = c("","NA"), col_names = TRUE)
...
invisible(lapply(chunk_files, validate_chunk_sra, positive_control = positive_control))

```

#Concatenate results chunks
```{r}
system("cat NCBI_search/taxIDs/NCBI_output_mito_03_species_singleNCBI_taxID_search_chunk_* > NCBI_search/taxIDs/NCBI_output_mito_03_species_singleNCBI_taxID_search_combined.txt")
system("cat NCBI_search/taxIDs/NCBI_output_mito_05_subspecies_singleNCBI_taxID_search_chunk_* > NCBI_search/taxIDs/NCBI_output_mito_05_subspecies_singleNCBI_taxID_search_combined.txt")
system("cat NCBI_search/taxIDs/NCBI_output_mito_06_genus_singleNCBI_taxID_search_chunk_* > NCBI_search/taxIDs/NCBI_output_mito_06_genus_singleNCBI_taxID_search_combined.txt")

system("cat NCBI_search/taxIDs/NCBI_output_sra_03_species_singleNCBI_taxID_search_chunk_* > NCBI_search/taxIDs/NCBI_output_sra_03_species_singleNCBI_taxID_search_combined.txt")
system("cat NCBI_search/taxIDs/NCBI_output_sra_05_subspecies_singleNCBI_taxID_search_chunk_* > NCBI_search/taxIDs/NCBI_output_sra_05_subspecies_singleNCBI_taxID_search_combined.txt")
system("cat NCBI_search/taxIDs/NCBI_output_sra_06_genus_singleNCBI_taxID_search_chunk_* > NCBI_search/taxIDs/NCBI_output_sra_06_genus_singleNCBI_taxID_search_combined.txt")
```

# Import NCBI mitochondrial and WGS results
```{r}
File03_taxid_search <- read_delim("NCBI_search/taxIDs/NCBI_output_mito_03_species_singleNCBI_taxID_search_combined.txt", col_types = "c", na = c("","NA"), col_names = c("AccessionVersion", "Title", "CreateDate", "Organism", "TaxId", "MolType", "Genome", "Completeness", "Topology", "Slen", "SubType", "SubName"))

File03_sra_search <- read_csv("NCBI_search/taxIDs/NCBI_output_sra_03_species_singleNCBI_taxID_search_combined.txt", col_types = cols(.default = col_character()), na = c("","NA"), col_names = TRUE)

File05_taxid_search <- read_delim("NCBI_search/taxIDs/NCBI_output_mito_05_subspecies_singleNCBI_taxID_search_combined.txt", col_types = "c", na = c("","NA"), col_names = c("AccessionVersion", "Title", "CreateDate", "Organism", "TaxId", "MolType", "Genome", "Completeness", "Topology", "Slen", "SubType", "SubName"))

File05_sra_search <- read_csv("NCBI_search/taxIDs/NCBI_output_sra_05_subspecies_singleNCBI_taxID_search_combined.txt", col_types = cols(.default = col_character()), na = c("","NA"), col_names = TRUE)

File06_taxid_search <- read_delim("NCBI_search/taxIDs/NCBI_output_mito_06_genus_singleNCBI_taxID_search_combined.txt", col_types = "c", na = c("","NA"), col_names = c("AccessionVersion", "Title", "CreateDate", "Organism", "TaxId", "MolType", "Genome", "Completeness", "Topology", "Slen", "SubType", "SubName"))

File06_sra_search <- read_csv("NCBI_search/taxIDs/NCBI_output_sra_06_genus_singleNCBI_taxID_search_combined.txt", col_types = cols(.default = col_character()), na = c("","NA"), col_names = TRUE)
...
#TODO: Check why no aphiaID is set taxonrank to species


#Local BOLD Searching
```{r}
vector_names <- UPDATED_FULL_RECORD_DB %>%
  select(Scientific.Name, scientificName, acceptedNameUsage, speciesNOsubgenus) %>%
  mutate(across(everything(), toupper)) %>%
  unlist(use.names = FALSE) %>%
  unique() %>%
  na.omit()

search_terms <- c("bold_recordset_code_arr", vector_names)
writeLines(search_terms, "BOLD_search_patterns.txt")

system(paste0('grep -iF -f BOLD_search_patterns.txt ',BOLD_local,' > BOLD_line_matches.txt'))

setwd(exportdir)
input_list_merge_records <- read.delim("export/FINAL_merge_records.txt", header=TRUE, stringsAsFactors=FALSE, na = c("","NA"))
UPDATED_FULL_RECORD_DB <- read.delim("export/scientificNameTable_FINAL_CompleteDatabase.txt", header=TRUE, stringsAsFactors=FALSE, na = c("","NA"))

```

#Remote BOLD Searching
```{r}
vector_names <- UPDATED_FULL_RECORD_DB %>%
  select(Scientific.Name, scientificName, acceptedNameUsage, speciesNOsubgenus) %>%
  mutate(across(everything(), toupper)) %>%
...
BOLD_names_db <- data.frame(Scientific.Name = vector_names)

bold_results <- bold_tax_name(BOLD_names_db$Scientific.Name, fuzzy = TRUE)
```
...
#TODO REMOVE POSITIVE CONTROLS FROM TABLE

#Merge scientific name table (UPDATED_FULL_RECORD_DB) with the original request table
```{r}
scientific_name_table_expanded <- UPDATED_FULL_RECORD_DB %>%
  separate_rows(row_ids, sep = ";")

input_list_merge_records <- input_list_clean %>%
  left_join(scientific_name_table_expanded, by = c("row_id" = "row_ids")) %>%
  mutate(
    Scientific.Name = if_else(
      Scientific.Name.x == Scientific.Name.y,
      coalesce(Scientific.Name.x, Scientific.Name.y),
      paste(Scientific.Name.x, Scientific.Name.y, sep = ";")
    )
  ) %>%
  select(-Scientific.Name.x, -Scientific.Name.y)

input_list_merge_records$row_id %>%  length()
input_list_merge_records$row_id %>% unique() %>%  length()
input_list_merge_records$AphiaID %>% unique() %>%  length()
input_list_merge_records$identifier %>% unique() %>%  length()

```

#Export the full record DB
```{r}
write.table(input_list_merge_records, file="export/FINAL_merge_records.txt", sep='\t', quote=FALSE, row.names=FALSE, col.names = TRUE)

write.table(UPDATED_FULL_RECORD_DB, file="export/scientificNameTable_FINAL_CompleteDatabase.txt", sep='\t', quote=FALSE, row.names=FALSE, col.names = TRUE)
...
for (taxon in possible_taxa_ranks) {
  ...
  output_file <- paste0("export/separateTaxonRanks/scientificNameTable_FINAL_", taxon, "Database.txt")
...
write.table(filtered_df, file="export/scientificNameTable_FINAL_SpeciesPlusSubDatabase.txt", sep='\t', quote=FALSE, row.names=FALSE, col.names=TRUE)

filtered_df <- subset(UPDATED_FULL_RECORD_DB, taxonRank %in% c("Genus", "Subgenus"))
write.table(filtered_df, file="export/scientificNameTable_FINAL_GenusPlusSubgenusDatabase.txt", sep='\t', quote=FALSE, row.names=FALSE, col.names=TRUE)

```

#Prioritization

#SHOULD ALSO INCLUDE prioritization based on proximity to mitogenome hit (same species, genus, family)
